...
Page({
  ...
  async loadRecentPlayers() {
    try {
      const recentPlayers = await getRecentPlayers(this.data.activeSport as any);
  ...
  chooseRecent(event: WechatMiniprogram.BaseEvent) {
    const player = this.data.recentPlayers.find((item: any) => item.id === Number(event.currentTarget.dataset.playerId));
    if (!player) {
      return;
    }
    this.applyPlayer({
